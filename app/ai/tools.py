...
FRAGRANCE_AGENT_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Deterministically score real DUA products against the customer's current profile using real order-history evidence (region, season/weather direction, likes/dislikes, repeat-purchase and popularity signals). Returns up to 10 real ProductCandidate results. Requires genuine discovery completeness first -- see get_customer_profile's description for the exact dimensions (name, style/vibe, dislikes-resolved, occasion-resolved, performance preference, location-resolved). A style direction plus only one other weak signal (e.g. 'strong oud' plus a passing mention of warm weather) is NOT enough on its own anymore -- calling this before every dimension is covered will fail with the specific thing still missing.",
            "parameters": {"type": "object", "properties": {}},
        },
    },
    {
